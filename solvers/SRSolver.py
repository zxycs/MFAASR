...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        self.records = {'train_loss': [],
                        ...
                        'lr': []}
        self.total_time = []
        self.model = create_model(opt)#创建模型
    ...
    def train_step(self):
        ...
        for i in range(self.split_batch):#self.split_batch这里设置为1
            loss_sbatch = 0.0
            split_LR = self.LR.narrow(0, i*sub_batch_size, sub_batch_size)
            split_HR = self.HR.narrow(0, i*sub_batch_size, sub_batch_size)
            if self.use_cl:#是否采用多个损失
                outputs = self.model(split_LR)#输出在这里，
                #split_LR的shape为torch.Size([16, 3, 40, 40])
                #outputs为list 其中每个元素为torch.Size([16, 3, 160, 160])
                loss_steps = [self.criterion_pix(sr, split_HR) for sr in outputs]#计算每个输出与HR之间的损失
                for step in range(len(loss_steps)):
                    loss_sbatch += self.cl_weights[step] * loss_steps[step]#如果有多个损失，则将这些损失乘以对应的权重后相加
            else:
                output = self.model(split_LR)
                # print(1111111111111111,output.size())
                # print(2222222222222222,split_HR.size())
                loss_sbatch = 4*self.criterion_pix(output, split_HR)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.self_ensemble and not self.is_train:
                ...
                t1 = time.time()
                # print(t1-t0)
                self.total_time.append((t1 - t0))
                # print("时间： ",t1-t0)

            if isinstance(SR, list):#判断如果是一个out为列表，则输出列表最后的那张图片
                self.SR = SR[-1]
            else:
                self.SR = SR

        # print( "平均时间：",sum(self.total_time)/len(self.total_time))
        self.model.train()
        if self.is_train:
    ...
    def load(self):
        ...
        if (self.is_train and self.opt['solver']['pretrain']) or not self.is_train:
            ...
            if self.is_train:
                ...
                load_func(checkpoint,False)
                #load_func(checkpoint, True)
