...
def define_net(opt):
    ...
    elif which_model.find('MSRN') >= 0:
        from .msrn_arch import MSRN
        net=MSRN()

    elif which_model.find('IMDN') >= 0:
        from .imdn_arch import IMDN
        net=IMDN()

    elif which_model.find('ZXYNET') >= 0:
        from .zxynet_arch import ZXYNET
        #print("11111111111111111111111111111111111111111111111111111")
        net = ZXYNET(in_channels=opt['in_channels'], out_channels=opt['out_channels'],
                                  num_features=opt['num_features'], num_steps=opt['num_steps'], num_groups=opt['num_groups'],
                                  upscale_factor=opt['scale'])

    elif which_model.find('MVPNET') >= 0:
        from .mvpnet_arch import MVPNET
        #print("222222222222222222222222222222222222222222222222222222222222")
        net = MVPNET(in_channels=opt['in_channels'], out_channels=opt['out_channels'],
                                  num_features=opt['num_features'], num_steps=opt['num_steps'], num_groups=opt['num_groups'],
                                  upscale_factor=opt['scale'])

    elif which_model.find('CSUNET') >= 0:
        from .csunet_arch import CSUNET
        #print("222222222222222222222222222222222222222222222222222222222222")
        net = CSUNET(in_channels=opt['in_channels'], out_channels=opt['out_channels'],
                                  num_features=opt['num_features'], num_steps=opt['num_steps'], num_groups=opt['num_groups'],
                                  upscale_factor=opt['scale'])

    elif which_model.find('CARN') >= 0:
        from .carn_arch import CARN
        net = CARN()
